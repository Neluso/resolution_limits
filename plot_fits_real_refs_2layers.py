...
for i in range(4):
    figure(i+1)
    loglog(sort(data[8 * i:8 * i + 7, 1]), sort(data[8 * i:8 * i + 7, 1]), 'k-', lw=1.0, label='sims')
    loglog(data[8 * i:8 * i + 7, 1], data[8 * i:8 * i + 7, 2], '.', label='inner')
    loglog(data[8 * i:8 * i + 7, 1], data[8 * i:8 * i + 7, 3], '.', label='outer')
    title(data[8 * i, 0])
    legend()
show()
