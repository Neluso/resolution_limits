...
def cost_function(params, *args):
    ...
    E_sam, E_ref_w, freqs, n_i, k_i, n_o, k_o, inc_angle, polaritz = args
    H_teo = H_sim(freqs, n_i, k_i, thick_i, n_o, k_o, thick_o, d_air, inc_angle, polaritz)
...
if __name__ == '__main__':
    for trace_idx in range(len(trace_list)):
        k_bounds = [
            (-1e-9, 1e-9),  # d_air
            (0, 1e-3),  # d_mat
            (0, 1e-3)  # d_mat
        ...
        resx1 = list()
        resx2 = list()
        inc_angle = int(trace_ID[trace_idx][:2])
        polaritz = trace_ID[trace_idx][2]
        for i in range(num_statistics):
            res = differential_evolution(cost_function,
                                         k_bounds,
                                         args=(trace_list[trace_idx], E_ref_w, f_ref,
        ...
        wh.write(str(inc_angle) + ' ' + polaritz + ' '
...
# plot(t_ref, irfft(H_teo_15s * E_ref_w), label='15s')
# plot(t_ref, irfft(H_teo_15p * E_ref_w), label='15p')
# plot(t_ref, irfft(H_teo_30s * E_ref_w), label='30s')
# plot(t_ref, irfft(H_teo_30p * E_ref_w), label='30p')
# plot(t_ref, irfft(H_teo_45s * E_ref_w), label='45s')
# plot(t_ref, irfft(H_teo_45p * E_ref_w), label='45p')
# plot(f_ref, toDb_0(H_teo_15s * E_ref_w), label='15s')
# plot(f_ref, toDb_0(H_teo_15p * E_ref_w), label='15p')
# plot(f_ref, toDb_0(H_teo_30s * E_ref_w), label='30s')
# plot(f_ref, toDb_0(H_teo_30p * E_ref_w), label='30p')
# plot(f_ref, toDb_0(H_teo_45s * E_ref_w), label='45s')
# plot(f_ref, toDb_0(H_teo_45p * E_ref_w), label='45p')
# plot(f_ref, unwrap(angle(H_teo_15s * E_ref_w)), label='15s')
# plot(f_ref, unwrap(angle(H_teo_15p * E_ref_w)), label='15p')
# plot(f_ref, unwrap(angle(H_teo_30s * E_ref_w)), label='30s')
# plot(f_ref, unwrap(angle(H_teo_30p * E_ref_w)), label='30p')
# plot(f_ref, unwrap(angle(H_teo_45s * E_ref_w)), label='45s')
# plot(f_ref, unwrap(angle(H_teo_45p * E_ref_w)), label='45p')
# legend()
# show()
