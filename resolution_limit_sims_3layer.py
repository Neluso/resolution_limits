...
def sim_traces(working_dir, e_s_sim_i, e_inf_sim_i, tau_sim_i,
               e_s_sim_m, e_inf_sim_m, tau_sim_m,
    ...
    for ref_file in ref_list:
        ...
        # e_inf_sim_m = 1.8**2
        # tau_sim_m = 1e-13
        e_s_sim_m = e_s_sim_m ** 2
        e_inf_sim_m = e_inf_sim_m ** 2
        ...
        for d_mat in pow(10, arange(-1, 3, 1/3)):  # 1/3
            ...
            name_trace = name_trace + str(e_s_sim_m) + '_' + str(e_inf_sim_m) + '_' + str(tau_sim_m) + '_'
            name_trace = name_trace + str(e_s_sim_o) + '_' + str(e_inf_sim_o) + '_' + str(tau_sim_o) + '_'
            name_trace = name_trace + ns_level + '.txt'

            d_mat *= 1e-6  # um
            
            H_sim_teo = H_sim(f_ref, n_sim_i, k_sim_i, d_mat, n_sim_m, k_sim_m, d_mat, n_sim_o, k_sim_o, d_mat, 0)  # - d_mat)
...
for line in sim_h:
    wk_dir, e_s_i, e_inf_i, tau_i, e_s_m, e_inf_m, tau_m, e_s_o, e_inf_o, tau_o = line.split(';')
    e_s_i = float(e_s_i)
    e_inf_i = float(e_inf_i)
    tau_i = float(tau_i)
    e_s_m = float(e_s_m)
    e_inf_m = float(e_inf_m)
    tau_m = float(tau_m)
    e_s_o = float(e_s_o)
    ...
    sim_traces(wk_dir, e_s_i, e_inf_i, tau_i, e_s_m, e_inf_m, tau_m, e_s_o, e_inf_o, tau_o)
    print()
